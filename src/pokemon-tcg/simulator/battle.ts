...
export class Battle extends EventEmitter{
	...
	constructor(data: any, room?: Room) {
		...
		this.battleId = data.battleid ? data.battleid : Tools.toId(this.format + new Date().getTime()) + Tools.random(99);
...
class _Battles {
	...
	challenge(from:User,to:User,format = "standard",deck:string,page:any) {
		...
		this.challenges[from.id] = {to:to.id,from:from.id,format,date:(new Date()).getTime(),fromdeck:deck,todeck:null, page:page};
	...
	acceptChallenge(from:User,to:User) {
		...
		let id = challenge.format + ((new Date()).getTime() + Tools.random(99).toString()).slice(5);

		if(!this.pages[from.id]) this.pages[from.id] = {}
		if(!this.pages[to.id]) this.pages[to.id] = {}

		this.pages[from.id][id] = new BattlePage(this.mainRoom, from, id, challenge)
		this.pages[to.id][id] = new BattlePage(this.mainRoom, to, id, challenge)
